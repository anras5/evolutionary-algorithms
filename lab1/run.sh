...
run_algorithm_ten_times() {
    M="$1"
    mkdir -p "f9-mut-$M/HoF/"
    mkdir -p "f9-mut-$M/Deap/"
    rm -f "f9-mut-$M/times.txt"
    echo "N,time" >> "f9-mut-$M/times.txt"
    ...
    for N in {1..10}; do
        ...
        uv run FramsticksEvolution.py \
            ...
            -hof_savefile "f9-mut-$M/HoF/f9-$N.gen" \
            -deap_logfile "f9-mut-$M/Deap/f9-$N.csv"

        # Wypisanie czasu wykonania algorytmu do times.txt
        end_time=$(date +%s)
        elapsed_time=$((end_time - start_time))
        echo "$N,${elapsed_time}" >> "f9-mut-$M/times.txt"
    done
