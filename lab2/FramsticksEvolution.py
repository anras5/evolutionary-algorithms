	...
	if valid and fitness[0] < 0:
		match parsed_args.function:
			...
			case 'zero':
				fitness = [0]
			case 'epsilon':
				fitness = [- np.exp(-0.1*np.pow(default_evaluation_data['numjoints'], 2))]
	return fitness
...
def parseArguments():
	...
	parser.add_argument('-function', type=str, default='default', choices=['default', 'zero', 'epsilon'], help="The function to assign to solutions with fitness lower than 0.")
