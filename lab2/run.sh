...
run_algorithm() {
    ...
    uv run FramsticksEvolution.py \
        ...
        -popsize 60 -generations 150 -hof_size 1 \
        -hof_savefile "lab2/f$G/HoF/$iteration.gen" \
        -deap_logfile "lab2/f$G/Deap/$iteration.csv"

    # Wypisanie czasu wykonania algorytmu do times.txt
    end_time=$(date +%s)
    elapsed_time=$((end_time - start_time))
    echo "${iteration},${elapsed_time}" >> "lab2/f$G/times.txt"
...
for G in "${G_values[@]}"; do
    mkdir -p "lab2/f$G/HoF/"
    mkdir -p "lab2/f$G/Deap/"
    rm -f "lab2/f$G/times.txt"
    echo "N,time" >> "lab2/f$G/times.txt"
