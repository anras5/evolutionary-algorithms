...
def is_feasible_fitness_value(fitness_value: float) -> bool:
    assert isinstance(
        fitness_value, float
    ), f"feasible_fitness({fitness_value}): argument is not of type float, it is of type {type(fitness_value)}"  # since we are using DEAP, we unfortunately must represent the fitness of an "infeasible solution" as a float...
    return (
            fitness_value != FITNESS_VALUE_INFEASIBLE_SOLUTION
...
def is_feasible_fitness_criteria(fitness_criteria: tuple) -> bool:
    return all(
        is_feasible_fitness_value(fitness_value) for fitness_value in fitness_criteria
    )
...
def select_feasible(individuals):
    ...
    feasible_individuals = [
        ind for ind in individuals if is_feasible_fitness_criteria(ind.fitness.values)
    ]
...
def selTournament_only_feasible(individuals, k, tournsize, change_mutation):
    global best_fitness, generation
    if change_mutation:
        best_fitness_population = max([individual.fitness for individual in individuals])
        if best_fitness is None:
            best_fitness = best_fitness_population
        elif best_fitness_population <= best_fitness:
            generation += 1
        elif best_fitness_population > best_fitness:
            best_fitness = best_fitness_population
            generation = 0
        ...
        if generation > 5:
            ...
            frams.GenMan.f1_smX = max(frams.GenMan.f1_smX._value() - 0.5, 1.0)
            frams.GenMan.f1_smJunct = max(frams.GenMan.f1_smJunct._value() - 0.5, 1.0)
            frams.GenMan.f1_smComma = max(frams.GenMan.f1_smComma._value() - 0.5, 1.0)
            frams.GenMan.f1_smModif = max(frams.GenMan.f1_smModif._value() - 0.5, 1.0)

            frams.GenMan.f1_nmNeu = frams.GenMan.f1_nmNeu._value() + 0.5
            frams.GenMan.f1_nmConn = frams.GenMan.f1_nmConn._value() + 0.5
            frams.GenMan.f1_nmWei = frams.GenMan.f1_nmWei._value() + 1.5
...
def prepareToolbox(
        frams_lib, OPTIMIZATION_CRITERIA, tournament_size, genetic_format, initial_genotype, change_mutation
):
    creator.create(
    ...
    if len(OPTIMIZATION_CRITERIA) <= 1:
        ...
        toolbox.register(
            "select", selTournament_only_feasible, tournsize=tournament_size, change_mutation=change_mutation
        )
    else:
...
def parseArguments():
    ...
    parser.add_argument(
        ...
        type=int,
        default=False,
        help="Change mutation parameters based on the best fitness value.",
    )
...
def main():
    ...
    stats.register(
        "avg",
        lambda fitness_criteria: filter_feasible_for_function(
            np.mean, fitness_criteria
        ),
    )
    stats.register(
        "stddev",
