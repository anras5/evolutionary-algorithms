    ...
    try:
        ...
        if fitness[0] < 0:
            match parsed_args.function:
                ...
                case "exp":
                    fitness = [
                        -np.exp(-0.1 * np.pow(default_evaluation_data["numjoints"], 2))
                    ]
                case "reciprocal":
...
def selTournament_only_feasible(individuals, k, tournsize, iteration, genetic_format):
    selected_individuals = tools.selTournament(select_feasible(individuals), k, tournsize=tournsize)

    # save genotypes
    global ITERATION
    save_genotypes(f"lab4/f{genetic_format}/genotypes/{iteration}/{ITERATION}.gen", OPTIMIZATION_CRITERIA,
...
def prepareToolbox(
        frams_lib, OPTIMIZATION_CRITERIA, tournament_size, genetic_format, initial_genotype, iteration
):
    creator.create(
    ...
    if len(OPTIMIZATION_CRITERIA) <= 1:
        ...
        toolbox.register(
            "select", selTournament_only_feasible, tournsize=tournament_size, iteration=iteration,
...
def parseArguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="default",
        choices=["default", "zero", "exp", "reciprocal"],
        help="The function to assign to solutions with fitness lower than 0.",
    )
    parser.add_argument(
        "-iteration",
        type=int,
