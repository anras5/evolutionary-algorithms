...
if __name__ == '__main__':
    ...
    for gen_format in [0, 1, 4, 9]:
        ...
        # divide the (min, max) range into 500 equal parts
        fitness_range = [min_fitness + (max_fitness - min_fitness) / 500 * i for i in range(501)]
        # save one individual for each of the 500 parts
        ...
        # mutate each individual in selected_individuals 20 times and save the fitness values
        results = {f"fitness_{i}": [] for i in range(1, 21)}
        results["fitness_original"] = []
        for ind in tqdm(selected_individuals):
            results["fitness_original"].append(ind.fitness)
            for i in range(1, 21):
                mutated = frams_mutate(framsLib, ind.genotype)
                results[f"fitness_{i}"].append(frams_evaluate(framsLib, [mutated[0]]))

        df = pd.DataFrame(results)
        df.to_csv(f"lab4/f{gen_format}/mutations.csv", index=False)
