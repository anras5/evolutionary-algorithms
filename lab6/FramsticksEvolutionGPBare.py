...
def gp_stick(genotype):
    return "X" + genotype if genotype is not None else "X"


def gp_parenthesis(genotype):
    return "(" + genotype + ")" if genotype is not None and "X" in genotype else None


def gp_comma(genotype1, genotype2):
    if genotype1 is not None and genotype2 is not None:
        return f"({genotype1},{genotype2})"
    if genotype1 is not None:
        return f"({genotype1},)"
...
def gp_modifier(modifier):
    def gp_modifier_inner(genotype):
        return modifier + genotype if genotype is not None else None
...
def build_pset():
    ...
    pset.addTerminal(None, Bare)
    pset.addTerminal(None, Branch)
